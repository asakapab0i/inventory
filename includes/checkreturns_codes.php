...
function checkYourReturns() {
	...
	if(isset($_POST['sample'])){
		...
		$count = $_POST['count'];
		$delivery_id = $_POST['submit_did'];
		//explode
		$newdelivery = explode(";",$delivery_id);
		$newpid = explode(";",$pid);
	...
	/* 	$sql = mysql_query("INSERT INTO returns 
		(supplier_name, 
		delivery_date, amount_paid,emp_id) 
		VALUES ('$supplier2',
		'$timestamp',
		'$total', 
		'$employee_id')") 
		...
		while($count > -1){
				...
				//inserting the data to returns table
				$sql2 = ("INSERT INTO returns_details
				(returns_id,
				product_id,
				delivery_id,
				returned_qty,
...
/*  	echo '<hr/>';
	...
	echo 'This is the delivery  '.$delivery_id.' ';
	}else
	echo '$_POST[\'submit_did\'] is empty';
	...
	echo '</div>';
	echo '<h3>Supplier Name: '.$supplier.' </h3>';
	echo '<h3>Delivery ID: '.$delivery_id.' </h3>';
...
echo '<hr/>';
	...
	<form name="pass_data"  class="container" method="post" action="checkreturns.php">
	...
	<input type="text" name="submit_did" value="'.$delivery_id.'"style="visibility:hidden;position:absolute">
	...
	<input type="text" name="supplier2" value="'.$supplier.'" style="visibility:hidden;position:absolute">

  <input type="submit" value="Submit Delivery" class="minibutton">
  </form>
</div>';
